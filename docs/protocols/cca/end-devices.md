...
1. **The encrypted Phoenix `firmware.tar.enc` PFF entries.** `cca/cca-eagle-owl-*.pff`, `cca/cca-bananaquit-avis-*.pff`, `cca/cca-basenji-*.pff` were enumerated in `device-firmware-manifest.json` (see [coproc.md](../../devices/coprocessor-firmware.md) §Device Firmware Manifest). These are the **actual end-device images** that get OTA'd to the device — but each `.pff` payload is encrypted with a **per-device-model AES key burned into the device's own bootloader at manufacture** (not on the processor; `PegasusLinkData.LinkDataKey` is empty). The `6cba80b2bf3cf2a63be017340f1801d8` (ATECC608 slot 6) key decrypts the *outer* `firmware.tar.enc` bundle — it does **not** decrypt the per-`.pff` payload (empirically it decrypts none of the 5 CCA PFF payloads). Getting plaintext requires recovering the per-model key from a device (SWD/JTAG dump) or reflashing hardware directly. See [../../devices/coprocessor-firmware.md](../../devices/coprocessor-firmware.md) §"PFF File Format".
