...
A decryptor maintained outside this repo implements the full pipeline. Running against `sysconfig32.dll`:
...
2. Pull delta/OTA builds from Lutron's firmware CDN (firmware CDN enumeration is documented outside this repo) — a job-specific or engineering release may ship the daughter-board firmware separately.
...
5. **Enumerate the firmware CDN** for `QSGR`, `grafikeye`, `qsg*` paths to find delta/OTA builds — firmware CDN enumeration is documented outside this repo. Job-specific or engineering releases may ship the daughter-board firmware separately.
